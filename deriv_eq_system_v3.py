        ...
        for t in irange:
            for u in irange:
                ...
                if INDEXATION == 3:
                    ...
                    s1 = "({})*d{}{}".format(l1, ind3to1(s,t,u), ind3to1(i,j,k))
                    s2 = "({})*d{}{}".format(l2, ind3to1(s,t,u), ind3to1(l,m,n))
                    s3 = "({})*d{}{}".format(l3, ind3to1(p,q,r), ind3to1(s,t,u))

                index1_d = DIM_WN * (ind3to1(i,j,k) - 1) + (ind3to1(s,t,u) - 1)
                index2_d = DIM_WN * (ind3to1(l,m,n) - 1) + (ind3to1(s,t,u) - 1)
                index3_d = DIM_WN * (ind3to1(s,t,u) - 1) + (ind3to1(p,q,r) - 1)
...
def calc():
    ...
    for I in range(1,7):
        for L in range(1,7):
            vec = [lm3(P, I, L) for P in range(1,7)]
            print(f"e{I} * e{L} = {vec}")
